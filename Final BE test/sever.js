...
app.post('/create-team/:userId', ensureDbConnection, (req, res) => {
  ...
  if (!teamName || !emails || !Array.isArray(emails) || emails.length === 0) {
      return res.status(400).json({ error: 'Team name and at least one email are required' });
  }

  // Bước 1: Xác định mối quan hệ và kiểm tra email
  const getPartnersQuery = 'SELECT partner_id FROM teammates WHERE user_id = ? UNION SELECT ? AS partner_id';
  db.query(getPartnersQuery, [userId, userId], (err, results) => {
      ...
      const getUserIdsQuery = 'SELECT id FROM users WHERE email IN (?) AND id IN (?)';
      db.query(getUserIdsQuery, [emails, allowedUserIds], (err, results) => {
          ...
          db.query(createTeamQuery, [teamName], (err, result) => {
              ...
              const addUsersQuery = 'INSERT INTO team_members (user_id, team_id) VALUES ?';
              ...
              db.query(addUsersQuery, [values], (err) => {
                  ...
                  res.status(201).json({ message: 'Team created and users added successfully', teamId });
              });
          });
      });
  });
