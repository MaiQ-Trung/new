                  ...
                  >
                    <div class="flex mt-2 justify-between">
                      ...
                      >
                        <button
                          type="button"
                          @click.stop="toggleModalS(project.id)"
        ...
        <nav class="px-4">
          <div class="flex flex-row justify-between">
            <ul class="flex font-medium space-x-3 text-gray-600">
              ...
              <li
                class="w-16"
                :class="{ 'text-blue-500': selectedNavItem === 'signed' }"
              >
                <a
                  href="#signed"
            ...
            >
              ...
              <template #option="{ value }">
                <div class="flex items-center" @click="signProject(value)">
                  <img
                    :src="users.find((user) => user.value === value).imageUrl"
                    alt="Profile Picture"
...
//fetch id and email of signed users
const fetchSignedUsers = async () => {
  for (const project of selectedProjects.value) {
    ...
    try {
      ...
      signedUsers.value = res.data.map((user) => ({
        id: user.id,
        email: user.email,
      }));
      value.value = signedUsers.value.map((user) => user.email);
      console.log(signedUsers.value);
    } catch (error) {
...
const signProject = async (email) => {
  ...
  for (const project of selectedProjects.value) {
    ...
    try {
      ...
      console.error(`Error signing project ${projectId} for ${email}:`, error);
    }
  }
};

// Remove signed user by id and project id
const removeUser = async (email) => {
  if (selectedProjects.value.length === 0) {
    console.log("No projects selected.");
    return;
  }
  const user = signedUsers.value.find((user) => user.email === email);
  if (!user) {
    console.error(`User ${email} not found in signed users`);
    return;
  }
  const id = user.id;
  for (const project of selectedProjects.value) {
    ...
    try {
      ...
      console.log(`User ${email} removed from project ${projectId}`);
    } catch (error) {
      console.error(
        `Error removing user ${email} from project ${projectId}:`,
        error
      );
    }
  }
...
const toggleModalS = async (projectId) => {
  await fetchSignedUsers(projectId);
