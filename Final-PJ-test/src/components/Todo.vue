      ...
      <div class="flex flex-row w-full">
        ...
        <div class="file-sidebar w-56 py-2 h-svh border-r-2 border-gray-300">
          <div class="button flex justify-end px-2 pb-2">
            ...
            <div
              v-if="showMenu"
              class="absolute left-52 top-20 bg-white border border-gray-200 shadow-lg w-52 min-h-24 p-2 flex flex-col rounded-lg"
            >
...
const fetchFiles = async () => {
  ...
  try {
    const query = await axios.get(`http://localhost:3000/files/${userId}`);
